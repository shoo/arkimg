 ...
 * 参照カウンタ
 */
module arkimg._internal.refcnt;
...
///
private template DelegateTypeOf(F)
if (isCallable!F)
{
...
///
private template DelegateTypeOf(alias F)
if (isCallable!F)
{
...
auto defaultAllocatorByMalloc(size_t bufSize = 0) pure nothrow @nogc @safe
{
	...
	static struct Alloc
	{
		...
		private CountedInstance* callImpl() nothrow
		{
			...
			auto buf = (cast(ubyte*)malloc(newSize))[0..newSize];
			...
			ret.deallocator = toDelegate((CountedInstance* inst)
			{
				...
				alias Fn = SetFunctionAttributes!(void function(void*), "C", FA.nogc | FA.nothrow_ | FA.pure_);
				(() @trusted => (cast(Fn)&GC.removeRange))()(inst);
				(() @trusted => (cast(Fn)&free ))()(inst);
			});
			return ret;
		}
		CountedInstance* opCall() pure nothrow
		{
			return (() @trusted => cast(CountedInstance* delegate() @nogc nothrow pure)&callImpl)()();
		}
	}
 ...
 */
RefCounted!T attachRefCounted(T)(T newRefData) @trusted
if (isCountedData!T)
{
	return (cast(RefCounted!T function(T*) pure @nogc nothrow)&attachRefCountedImpl!T)(&newRefData);
}
/// ditto
RefCounted!T attachRefCounted(T)(RefType!T newRefData) @trusted
if (!isCountedData!T)
{
	return (cast(RefCounted!T function(RefType!T*) pure @nogc nothrow)&attachRefCountedImpl!T)(&newRefData);
}

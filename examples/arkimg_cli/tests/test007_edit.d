...
void main()
{
	...
	result = execArkimgCli(["encrypt", "-i", testFile, "-s", scrFile, "--comment", "test of encrypt",
		"-k", key, "-v", "-f"]);
	assert(result.compareStrings(i`
		[info] Load $(testFile)
		[info] Add $(scrFile.baseName) (text/x-dsrc) at 1
		[info] Save $(testFile)
	`));
	...
	version (Windows)
		...
		enum errCode = "255";
	result = execArkimgCliFail(["edit", "-i", testFile, "-s=test.txt", "--comment", "TEST", "-k", key, "-v"]);
	assert(result.compareStrings(i`
		[warning] $(testFile) is existing.
		Error Program exited with code $(errCode)
	`));
	
	auto testOutFile = tempDir.uniqueName(".png");
	result = execArkimgCli([
		"edit", "-i", testFile, "-s=test.txt", "-o", testOutFile, "--comment", "TEST", "-k", key, "-v"]);
	assert(result.compareStrings(i`
		[info] Load $(testFile)
		[info] Edit at 0
	...
	result = execArkimgCli(["list", "-i", testOutFile, "-d", "-k", key, "-v"]);
	assert(result.compareStrings(i`
		test.txt
		    Mime:      text/plain
		...
		$(scrFile.baseName)
		    Mime:      text/x-dsrc
		    Digest:    $( scrFile.readText().sha256Of().toHexString() )
		    Modified:  $( scrFile.timeLastModified.toISOExtString() )
		    Comment:   test of encrypt
		`));
}

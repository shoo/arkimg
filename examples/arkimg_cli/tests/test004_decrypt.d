module examples.arkimg_cli.tests.test004_decrypt;
...
void main()
{
	...
	auto testOutFile = tempDir.uniqueName(".txt");
	result = execArkimgCli(["decrypt", "-i", testFile, "-s", "test.txt", "-o", testOutFile, "-d", "-k", key, "-v"]);
	assert(result.compareStrings(i`
		[info] Load $(testFile)
		[info] Found test.txt

...
void main()
{
	...
	version (Windows)
		...
		enum errCode = "255";
	result = execArkimgCliFail(["rm", "-i", testFile, "-s=test.txt", "-k", key, "-v"]);
	assert(result.compareStrings(i`
	...
	result = execArkimgCli(["rm", "-i", testFile, "-s=test.txt", "-o", testOutFile, "-k", key, "-v"]);
	assert(result.compareStrings(i`
		[info] Load $(testFile)

...
int removeCommand(string[] args)
{
	...
	try
	{
		auto getoptres = args.getopt(
			...
			std.getopt.config.required, "secret|s",
				"Specify secret data file name or index includeing arkimg to remove.",
				&secretFileName,
	...
	{
		warningf("%s is existing.", outputFileName);
		return -1;
	}
	...
	// メイン処理実行
	try remove(inputFileName, secretFileName, outputFileName, commonKey, iv, prvKeyDER);
	catch (Exception e)
	{
 ...
 * 
 */
void remove(ArkImg img, size_t secretFileIdx,
	in ubyte[] key, in ubyte[] iv = null, in ubyte[] prvKey = null)
{
	...
	enforce(img.getSecretItemCount() > 0);
	foreach (idx; 0..img.getSecretItemCount)
		oldSecrets ~= img.getDecryptedItem(idx);
	img.clearSecretItems();
	auto newSecrets = oldSecrets.remove(secretFileIdx);
	if (newMetadata !is JSONValue.init)
		if (auto items = "items" in newMetadata)
			items.array = items.array.remove(secretFileIdx);
	foreach (secretDat; newSecrets)
		img.addSecretItem(secretDat, prvKey: prvKey);
	if (newMetadata !is JSONValue.init)
		img.metadata = newMetadata;
}
/// ditto
void remove(string arkimgFile, string secretFileName, string outFileName,
	in ubyte[] key, in ubyte[] iv = null, in ubyte[] prvKey = null)
{
	import std.exception;
	import std.file;
	...
	enforce(prvKey.length == 0 || img.verify(prvKey.createPublicKey()));

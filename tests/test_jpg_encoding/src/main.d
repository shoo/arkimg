...
void main()
{
	...
	auto imgPng = loadImage("../.resources/d-man.png", key);
	std.file.write("test1.bmp", imgPng.baseImage("image/bmp"));
	auto bufPng2Jpeg = saveImage(imgPng, "image/jpeg", key);
	std.file.write("test2.jpg", bufPng2Jpeg);
	auto imgJpg2 = loadImage("test2.jpg");
	auto bufJpg2Png = saveImage(imgJpg2, "image/png", key);
	std.file.write("test3.png", bufJpg2Png);
	auto imgPng2 = loadImage(bufJpg2Png, "image/png", key);
	auto bmpImg3 = createBitmap(imgPng2.baseImage("image/bmp"));
	
	diff = 0.0L;
	assert(bmpImg3.row(0).length == bmpImg2.row(0).length);
